...
function openPanel(key=null,trigger){
    ...
    if(!key){
        ...
        const p=projects[key];$('#panel-content').innerHTML=`<button class="back-projects" id="back-projects">all the detours</button><h2 id="panel-title">${p.title}</h2><p>${p.description}</p>${p.detail?`<p>${p.detail}</p>`:""}${p.scene?`<iframe class="scene-frame" src="demo.html?scene=${p.scene}" title="${p.title} interactive illustration" loading="eager"></iframe>${p.note?`<p class="scene-note">${p.note}</p>`:""}`:""}${(p.links||[]).map(link=>`<a class="more-scene" href="${link.url}" target="_blank" rel="noopener noreferrer">${link.label}</a>`).join('')}`;
