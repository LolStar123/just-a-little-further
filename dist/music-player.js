...
export function musicPlayer(mix){
    ...
    controls.innerHTML=`<label>music <input id="music-volume" type="range" min="0" max="100" step="0.001" value="6.615" aria-label="Music volume"></label><label>sfx <input id="sfx-volume" type="range" min="0" max="100" value="48" aria-label="Sound effects volume"></label><span id="audio-status" role="status"></span>`;
    document.querySelector('#world').prepend(controls);
    const status=controls.querySelector('#audio-status');
    let volume=.06615,part=0,slot=0,blocked=false,error=null,switching=false,fadeEnd=0,fromSlot=0,prepared=-1;
