...
export function attachToys(canvas,wake,sfx){
    ...
    const world={actors:new Map(),props:new Map(),held:null,wake,sfx,last:0,throws:0};worlds.set(canvas,world);
    ...
    canvas.addEventListener('pointerdown',e=>{
        ...
        const items=[...world.actors.values(),...world.props.values()].filter(item=>performance.now()-item.lastFrame<200).reverse();
        for(const item of items){
            const p=point(e,item),box=item.box;
            if(p.x<item.x-box.w/2||p.x>item.x+box.w/2||p.y<item.y-box.h||p.y>item.y+8)continue;
    ...
    canvas.addEventListener('pointermove',e=>{
        ...
        const x=clamp(p.x-item.grip.x,12,item.width-12),y=clamp(p.y-item.grip.y,25,item.height-6);
        item.vx=clamp((x-item.x)/dt,-550,550);item.vy=clamp((y-item.y)/dt,-600,600);item.x=x;item.y=y;item.last=now;wake();e.preventDefault();
    });
    ...
    canvas.__toys=()=>({throws:world.throws,held:world.held?.id,actors:[...world.actors.values()].map(({id,x,y,home,active,mode})=>({id,x,y,home,active,mode})),props:[...world.props.values()].filter(item=>performance.now()-item.lastFrame<200).map(({id,x,y,active,box})=>({id,x,y,active,box}))});
}
function itemFor(c,id,x,y,w,h,actor,ground){
    ...
    const bank=actor?world.actors:world.props;let item=bank.get(id);
    if(!item){item={id,x,y,vx:0,vy:0,active:false,mode:'rest',lastFrame:performance.now()};bank.set(id,item);}
...
export function toyPose(c,id,x,y,size,options){
...
export function toyProp(c,id,x,y,w,h,paint){
    const result=itemFor(c,id,x,y,w,h,false);c.save();
    if(result){const local=new DOMPoint(result.item.x,result.item.y).matrixTransform(result.matrix.inverse());c.translate(local.x-x,local.y-y);}
    paint();c.restore();
